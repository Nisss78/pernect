...
import { useResultHistoryData } from "../hooks/useResultHistoryData";
...
export default function ResultHistoryList({
  ...
  const { results } = useResultHistoryData();
  ...
  const renderIcon = (icon?: string) => {
    const name = icon || "";
    if (name === "brain") {
      return <MaterialCommunityIcons name="brain" size={24} color="white" />;
    }
    const ionNameMap: Record<string, keyof typeof Ionicons.glyphMap> = {
      briefcase: "briefcase",
      heart: "heart",
      star: "star",
      book: "book",
    ...
    const ionName = ionNameMap[name] || "help-circle";
    return <Ionicons name={ionName} size={24} color="white" />;
  };
